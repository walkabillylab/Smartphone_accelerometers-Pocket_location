...
# To extract raw data for each participamnts and save it is a csv file
# This file does not change the frequency
...
for (dev_id in dev_ids) {
  ...
  for (mon in mons) {
    ...
    Selected_part$userid %>% map(function(i) {
      ...
      fwrite(x = df_slice, file = paste0(p_id, "_", wear_loc, "_raw.csv"))
      setwd(dir = main_path)
    })
  }
}
