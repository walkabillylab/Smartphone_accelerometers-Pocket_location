...
working_df %<>%  dplyr::select(c(participant_id, x_axis, y_axis, z_axis, Counts_vec_mag, height, weight, age,gender, trimmed_activity))
...
working_df %>% group_by(participant_id) %>% 
    select(-c(gender,trimmed_activity)) %>% 
    summarise_all(mean) %>%  
    kable(label = " Features mean for each participant") %>%
...
# Plot Mean for all
```{r}
working_df %>% select(-c(gender,trimmed_activity)) %>% 
  summarise_all(mean) %>% 
  stack() %>% 
