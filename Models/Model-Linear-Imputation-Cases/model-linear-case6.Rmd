...
## Linear imputation, Counts, metadata , and Few features

In this markdown, we use the linear imputed data. **Counts, metadata , and Few features** variables are used for this model. We read the CSV file which contains x,y and z `Counts` and then we can apply the following models using the `ApplyModels` function from `BEAP` package:
...
working_df %<>%  dplyr::select(c(x_axis, y_axis, z_axis, height, weight, age,gender, Counts_vec_mag, trimmed_activity))
