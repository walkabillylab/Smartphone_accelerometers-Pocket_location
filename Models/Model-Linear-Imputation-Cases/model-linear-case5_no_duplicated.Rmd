...
## Linear imputation, X, Y, Z, metadata, and new features

In this markdown, we use the linear imputed data. **X, Y, Z, metadata, and new features** variables are used for this model. We read the CSV file which contains x,y and z `Counts` and then we can apply the following models using the `ApplyModels` function from `BEAP` package:
...
# reduce the size of working_df slightly to add new_features
working_df  %<>% slice(1:nrow(new_features_df))
## model uses new_features too
...
## New section- Removing duplicates
Having duplicated values can affect the accuracy and results in false high accuracy.
