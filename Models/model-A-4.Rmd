...
## Linear imputation, X,Y,Z,metadata, Counts, and new features

In this markdown, we use the linear imputed data. **X, Y, Z-axis, metada, `counts` and new features** variables are used for this model. We read the CSV file which contains x,y and z `Counts` and then apply the following models using the `ApplyModels` function from `BEAP` package:
